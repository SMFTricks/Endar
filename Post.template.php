...
function template_main()
{
	...
	echo '
		<div id="preview_section"', isset($context['preview_message']) ? '' : ' style="display: none;"', '>
			<div class="border-mod">
					<div class="subtitle" id="preview_subject">', empty($context['preview_subject']) ? '' : $context['preview_subject'], '</div>
				<div class="row1">
	...
	echo '
							<div', empty($context['post_error']['messages']) ? ' style="display: none"' : '', ' class ="row1" id="errors">
							  
									<p style="', empty($context['error_type']) || $context['error_type'] != 'serious' ? 'display: none;' : '', '" id="error_serious" class="row">
										<strong>', $txt['error_while_submitting'], '</strong>
									</p>
									<p style="color: #990000; margin: 1ex 0 1ex 3ex;" id="error_list">
	...
	echo '
							<div', $context['locked'] ? '' : ' style="display: none"', ' class ="row1" id="lock_warning">
								<p><strong>', $txt['smf287'], '</strong></p>
								</div>';
echo'<div class="row1">';
...
<div class="row" style="', isset($context['post_error']['long_name']) || isset($context['post_error']['no_name']) || isset($context['post_error']['bad_name']) ? 'color: #990000;' : '', '" id="caption_guestname"><strong>', $txt[68], ': &nbsp;&nbsp;<input type="text" name="guestname" size="25" value="', $context['name'], '" tabindex="', $context['tabindex']++, '" />
</div>';
if (empty($modSettings['guest_post_no_email']))
			echo '<div class="row" style="font-weight: bold;', isset($context['post_error']['no_email']) || isset($context['post_error']['bad_email']) ? 'color: #990000;' : '', '" id="caption_email">
									', $txt[69], ': &nbsp;&nbsp;<input type="text" name="email" size="25" value="', $context['email'], '" tabindex="', $context['tabindex']++, '" />
								</div>';
	...
	if ($context['make_event'])
	{
		echo '
								...
								<span class="formleft" style="font-weight: bold;', isset($context['post_error']['no_event']) ? 'color: #990000;' : '', '" id="caption_evtitle">
									<strong>', $txt['calendar12'], '</strong>
								</span>
								<span class="formright">
									<input type="text" name="evtitle" maxlength="30" size="30" value="', $context['event']['title'], '" tabindex="', $context['tabindex']++, '" />
								</span>
						<br />
						<span class="formleft"><!--no content--></span>
								<span class="formright">
		...
		if (!empty($modSettings['cal_allowspan']))
		{
			echo '
							<br />
								<span class="formleft"><stong>', $txt['calendar54'], '</strong></span>
								<spanclass="formright">
									<select name="span">';
		...
		if ($context['event']['new'] && $context['is_new_post'])
		{
			echo '
							<br />
								<span class="formleft"><strong>', $txt['calendar13'], '</strong></span>
								<span class="formright">
									<select name="board">';
	...
	echo '
								...
								<div  class="row" style="', isset($context['post_error']['no_subject']) ? 'color: #990000;' : '', '" id="caption_subject">
									<strong>', $txt[70], ':</strong> <input type="text" name="subject"', $context['subject'] == '' ? '' : ' value="' . $context['subject'] . '"', ' tabindex="', $context['tabindex']++, '" size="80" maxlength="80" />
								</div></div>';

	echo'<div style="display:none;"><select name="icon" id="icon" onchange="showimage()">';
	...
	if ($context['make_poll'])
	{
		echo '
						...
						<div class="subtitle" style="', isset($context['post_error']['no_question']) ? 'color: #990000;' : '', '" id="caption_question">
								<strong>', $txt['smf21'], ':</strong> <input type="text" name="question" value="', isset($context['question']) ? $context['question'] : '', '" tabindex="', $context['tabindex']++, '" size="80" />
								</div>
						<div class="row1">
							<ul class="row-list">';
		...
		{
			echo '<li><label for="options-', $choice['id'], '">', $txt['smf22'], ' ', $choice['number'], '</label>: <input type="text" name="options[', $choice['id'], ']" id="options-', $choice['id'], '" value="', $choice['label'], '" tabindex="', $context['tabindex']++, '" size="25" />';
	  ...
	  <div class="subtitle"><strong>', $txt['poll_options'], ':</strong></div>
	 <div class="row1">
	 <div class="left">
	  <p class="smalltext"><input type="text" name="poll_max_votes" size="2" value="', $context['poll_options']['max_votes'], '" /> ', $txt['poll_options5'], '</p>
	  <p class="smalltext">', $txt['poll_options1a'], ' <input type="text" id="poll_expire" name="poll_expire" size="2" value="', $context['poll_options']['expire'], '" onchange="pollOptions();" /> ', $txt['poll_options1b'], '</p>
	 <p class="smalltext"><label for="poll_change_vote"><input type="checkbox" id="poll_change_vote" name="poll_change_vote" ', !empty($context['poll_options']['change_vote']) ? 'checked="checked"' : '', ' class="check" /> ', $txt['poll_options7'], '</label></p>
  </div>
  <div class="right">
  <p class="smalltext"><input type="radio" id="poll_hide" name="poll_hide" value="0"', $context['poll_options']['hide'] == 0 ? ' checked="checked"' : '', ' class="check" /> ', $txt['poll_options2'], '</p>
  <p class="smalltext"><input type="radio" id="poll_hide" name="poll_hide" value="1"', $context['poll_options']['hide'] == 1 ? ' checked="checked"' : '', ' class="check" /> ', $txt['poll_options3'], '</p>
  <p class="smalltext"><input type="radio" id="poll_hide" name="poll_hide" value="2"', $context['poll_options']['hide'] == 2 ? ' checked="checked"' : '', empty($context['poll_options']['expire']) ? ' disabled="disabled"' : '', ' class="check" /> ', $txt['poll_options4'], '</p>
								</div>
						</div>';
	}
	...
	if (isset($context['last_modified']))
		echo '
									<p><em>', $txt[211], ': ', $context['last_modified'], '</em></p>';

	// If the admin has enabled the hiding of the additional options - show a link and image for it.
	if (!empty($settings['additional_options_collapsable']))
		echo '
							<div class="subtitle">
							<a href="javascript:swapOptions();"><img src="', $settings['images_url'], '/expand.gif" alt="+" border="0" id="postMoreExpand" class="subtitle-collapse-icon" /></a><a href="javascript:swapOptions();"><strong>', $txt['post_additionalopt'], '</strong></a>
										</div>
							  ';

	// Display the check boxes for all the standard options - if they are available to the user!
	echo '<div class="row1" id="postMoreOptions">
<div class="left">
', $context['can_notify'] ? '<p class="smalltext"><input type="hidden" name="notify" value="0" /><label for="check_notify"><input type="checkbox" name="notify" id="check_notify"' . ($context['notify'] || !empty($options['auto_notify']) ? ' checked="checked"' : '') . ' value="1" class="check" /> ' . $txt['smf14'] . '</label></p>' : '', '
', $context['can_lock'] ? '<p class="smalltext"><input type="hidden" name="lock" value="0" /><label for="check_lock"><input type="checkbox" name="lock" id="check_lock"' . ($context['locked'] ? ' checked="checked"' : '') . ' value="1" class="check" /> ' . $txt['smf15'] . '</label>' : '</p>', '
<p class="smalltext"><label for="check_back"><input type="checkbox" name="goback" id="check_back"' . ($context['back_to_topic'] || !empty($options['return_to_post']) ? ' checked="checked"' : '') . ' value="1" class="check" /> ' . $txt['back_to_topic'] . '</label></p>
', $context['can_sticky'] ? '<p class="smalltext"><input type="hidden" name="sticky" value="0" /><label for="check_sticky"><input type="checkbox" name="sticky" id="check_sticky"' . ($context['sticky'] ? ' checked="checked"' : '') . ' value="1" class="check" /> ' . $txt['sticky_after2'] . '</label></p>' : '', '
</div>
<div class="right">
<p class="smalltext"><label for="check_smileys"><input type="checkbox" name="ns" id="check_smileys"', $context['use_smileys'] ? '' : ' checked="checked"', ' value="NS" class="check" /> ', $txt[277], '</label></p>', '
', $context['can_move'] ? '<p class="smalltext"><input type="hidden" name="move" value="0" /><label for="check_move"><input type="checkbox" name="move" id="check_move" value="1" class="check" /> ' . $txt['move_after2'] . '</label>' : '</p>', '', $context['can_announce'] && $context['is_first_post'] ? '
<p class="smalltext"><label for="check_announce"><input type="checkbox" name="announce_topic" id="check_announce" value="1" class="check" /> ' . $txt['announce_topic'] . '</label></p>
' : '', '
</div>
<div class="clear"><!--no content--></div>
	...
	if ($context['can_post_attachment'])
	{
		...
		if (!empty($modSettings['attachmentCheckExtensions']))
			...
			echo '
									<p class="smalltext">', $txt['smf120'], ': ', $context['allowed_extensions'], '<br />';
		echo '
	...
	echo '			 
									<div class="row1-center"><p class="smalltext">', $txt['smf16'], '</p></div>
									<div class="subtitle" style="text-align:center;"><input type="submit" class="button" name="post" value="', $context['submit_label'], '" tabindex="', $context['tabindex']++, '" onclick="return submitThisOnce(this);" accesskey="s" />
									<input type="submit" class="button" name="preview" value="', $txt[507], '" tabindex="', $context['tabindex']++, '" onclick="return event.ctrlKey || previewPost();" accesskey="p" />';

	// Option to delete an event if user is editing one.
	if ($context['make_event'] && !$context['event']['new'])
		echo '
									<input type="submit" class="button" name="deleteevent" value="', $txt['calendar22'], '" onclick="return confirm(\'', $txt['calendar21'], '\');" />';
	...
	if (isset($context['previous_posts']) && count($context['previous_posts']) > 0)
	{
		echo '
			...
			<div class="border">
			<h4 class="title"><strong>', $txt[468], '</strong></h4>
						<span id="new_replies"></span>';
		foreach ($context['previous_posts'] as $post)
			echo '
				 <h5 class="subtitle">
					<div class="float-r">', $txt[280], ': ', $post['time'], $post['is_new'] ? ' <img src="' . $settings['images_url'] . '/' . $context['user']['language'] . '/new.gif" alt="' . $txt['preview_new'] . '" border="0" />' : '', '</div>
									', $txt[279], ': ', $post['poster'], '
								</h5>
							<div class="row1" >
								<div class="smalltext" id="msg', $post['id'], '">
						', $post['message'], '
									<div style="text-align:right;"><a href="#top" onclick="return insertQuoteFast(', $post['id'], ');">', $txt[260], '</a></div>
								  </div>
								</div>';
		echo '
...
echo'<div class="row2">';
	...
	if (!empty($context['smileys']['postform']))
	{
		...
		foreach ($context['smileys']['postform'] as $smiley_row)
		{
			foreach ($smiley_row['smileys'] as $smiley)
				echo '
					<a href="javascript:void(0);" onclick="replaceText(\' ', $smiley['code'], '\', document.forms.', $context['post_form'], '.', $context['post_box_name'], '); return false;"><img src="', $settings['smileys_url'], '/', $smiley['filename'], '" align="bottom" alt="', $smiley['description'], '" title="', $smiley['description'], '" border="0" /></a>';
	...
	echo '
			</div><div class="row1-center">
					<div style="width:100%;"><textarea class="editor" name="', $context['post_box_name'], '" rows="', $context['post_box_rows'], '" cols="', $context['post_box_columns'], '" onselect="storeCaret(this);" onclick="storeCaret(this);" onkeyup="storeCaret(this);" onchange="storeCaret(this);" tabindex="', $context['tabindex']++, '"', isset($context['post_error']['no_message']) || isset($context['post_error']['long_message']) ? ' style="border: 1px solid #990000;"' : '', '>', $message, '</textarea></div></div>
			 ';
}
...
function template_announce()
{
	...
	echo '
		<form action="', $scripturl, '?action=announce;sa=send" method="post" accept-charset="', $context['character_set'], '">
			<div class="border">
				<h2 class="title">', $txt['announce_title'], '</h2>
				<div class="subtitle"><p>', $txt['announce_desc'], '</p></div>
				<div class="row1">
				 <p>', $txt['announce_this_topic'], ' <a href="', $scripturl, '?topic=', $context['current_topic'], '.0">', $context['topic_subject'], '</a><br /></p>
<ul class="bullit-list">';

	foreach ($context['groups'] as $group)
				echo '
						<li><label for="who_', $group['id'], '"><input type="checkbox" name="who[', $group['id'], ']" id="who_', $group['id'], '" value="', $group['id'], '" checked="checked" class="check" /> ', $group['name'], '</label> <em>(', $group['member_count'], ')</em></li>';

	echo '
						</ul>
<br />
						<p><label for="checkall"><input type="checkbox" id="checkall" class="check" onclick="invertAll(this, this.form);" checked="checked" /> <em>', $txt[737], '</em></label></p>
					</div>
				<div class="subtitle" style="text-align:center;">
						<input type="submit" class="button" value="', $txt[105], '" />
					</div>
<div class="block-foot"><!--no content--></div>
				</div>
			<input type="hidden" name="sc" value="', $context['session_id'], '" />
...
function template_announcement_send()
{
	...
	echo '
		<form action="' . $scripturl . '?action=announce;sa=send" method="post" accept-charset="', $context['character_set'], '" name="autoSubmit" id="autoSubmit">
			<div class="border">
				<h2 class="title">', $txt['announce_sending'], ' <a href="', $scripturl, '?topic=', $context['current_topic'], '.0" target="_blank">', $context['topic_subject'], '</a></h2>
					<div class="row1"><p><br /><strong>', $context['percentage_done'], '% ', $txt['announce_done'], '</strong><br /><br /></p></div>
				</div>
					<div class="subtitle" style="text-align:center;">
						<input type="submit" name="b" class="button" value="', $txt['announce_continue'], '" />
					</div>
				<div class="block-foot"><!-no content--></div>
			</div>
			<input type="hidden" name="sc" value="', $context['session_id'], '" />
